    ...
    try {
      if (isInWishlist) {
        ...
        if (wishlistItemId) {
          ...
          toast.error("Could not remove item: ID not found");
        }
      } else {
  ...
  if (error || !product)
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-center px-6">
        <h2 className="text-2xl font-bold text-[#501F08] mb-4">
          Masterpiece Not Found
        </h2>
        <p className="text-gray-500 mb-8 italic">
          "True beauty is sometimes elusive. Please check back later."
        </p>
        <button
          onClick={() => navigate("/products")}
          className="px-8 py-3 bg-[#501F08] text-white rounded-xl font-bold text-xs tracking-widest uppercase shadow-xl"
        >
  ...
  return (
    <div className="bg-[#FDFDFD] min-h-screen">
      <div className="max-w-screen-2xl mx-auto px-6 md:px-12 lg:px-24 py-12">
        {/* Breadcrumbs */}
        <div className="flex items-center gap-2 text-[10px] md:text-xs font-bold text-[#A87453] uppercase mb-6 md:mb-12 overflow-x-auto whitespace-nowrap pb-2 md:pb-0">
          <span
            className="hover:text-[#501F08] cursor-pointer transition-colors"
            onClick={() => navigate("/")}
          >
            Home
          </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-20">
          ...
          <div className="lg:col-span-7 flex flex-col-reverse md:flex-row gap-4 md:gap-6">
            {/* Thumbnails */}
            <div className="flex flex-row md:flex-col gap-3 md:gap-4 overflow-x-auto md:overflow-y-auto pb-2 md:pb-0 scrollbar-hide shrink-0 w-full md:w-auto md:max-w-[100px] items-start md:items-center pt-1.5 px-1">
              {product.images?.map((img, idx) => (
                <button
                  key={idx}
                  onClick={() => setActiveImage(idx)}
                  className={`relative w-20 h-20 rounded-2xl overflow-hidden border-2 transition-all duration-300 bg-white ${activeImage === idx ? "border-[#501F08] shadow-lg scale-105" : "border-gray-100 opacity-60 hover:opacity-100"}`}
                >
                  <img
                    src={img}
          ...
          <div className="lg:col-span-5 flex flex-col pt-4">
            ...
            <h1 className="text-3xl md:text-4xl lg:text-5xl font-black text-[#1A1A1A] mb-4 leading-tight">
              {product.name}
            </h1>
            ...
            <div className="space-y-8 mb-12">
              <div>
                <h3 className="text-xs font-black text-[#501F08] uppercase tracking-widest mb-4">
                  Description
                </h3>
                <div
                  className="text-gray-500 text-sm md:text-base leading-relaxed italic [&_p]:mb-4 [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5 [&_li]:mb-2 [&_strong]:font-bold [&_em]:italic"
              ...
              {cartItem && (
                <div className="flex items-center gap-8 animate-fade-in">
                  <h3 className="text-xs font-black text-[#501F08] uppercase tracking-widest">
                    Quantity
                  </h3>
                  <div className="flex items-center bg-[#FAF7F5] rounded-xl p-1 border border-[#501F08]/5">
                    <button
                      ...
                      className="p-2.5 text-[#501F08] hover:bg-white rounded-lg transition-all disabled:opacity-20"
                    >
                      <Minus className="w-4 h-4" />
                    </button>
                    <span className="w-12 text-center text-sm font-black text-[#501F08]">
                      {cartItem.qty}
                    </span>
                    <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-12">
              {!cartItem ? (
                <button
                  ...
                  className="w-full bg-[#501F08] text-white py-5 rounded-2xl font-black text-[12px] shadow-[0_20px_40px_rgba(80,31,8,0.2)] hover:shadow-[0_25px_50px_rgba(80,31,8,0.3)] transition-all flex items-center justify-center gap-3 uppercase"
                >
                ...
                <button
                  ...
                  className="w-full bg-[#501F08] text-white py-4 rounded-2xl font-black text-[12px] shadow-[0_20px_40px_rgba(80,31,8,0.2)] hover:shadow-[0_25px_50px_rgba(80,31,8,0.3)] transition-all flex items-center justify-center gap-3 uppercase"
                >
                  <ShoppingCart className="w-5 h-5" />
                  Go to Cart
                </button>
              )}
              <button className="relative group w-full overflow-hidden bg-transparent border-2 border-[#501F08] text-[#501F08] py-4 rounded-2xl font-black text-[14px] transition-all duration-300 hover:text-white uppercase flex items-center justify-center shadow-xl hover:shadow-2xl">
